...
/**
 * Created by change on 2018/9/30.
 */
...
public class ColumnView extends View {
    ...
    public void init(){
        //计算原点坐标和单位长度
        xPoint = margin + marginX;// 50
        yPoint = this.getHeight() - margin;//540-20
        xScale = (this.getWidth() - 2 * margin - marginX) / (transverse.length - 1);//w990  920/7  =131
        yScale = (this.getHeight() - 2 * margin) / (vertical.length - 1);// h 540/4  //125

        paintAxes = new Paint();     //画轴(横轴和竖轴)
        paintAxes.setStyle(Paint.Style.STROKE);
        paintAxes.setAntiAlias(true);
        paintAxes.setDither(true);
        paintAxes.setColor(colors.get(0));
        paintAxes.setStrokeWidth(1);

        paintCoordinate = new Paint(Paint.ANTI_ALIAS_FLAG);    //画坐标（文字或数值）
        paintCoordinate.setColor(colors.get(1));
        paintCoordinate.setTextSize(22f);

        paintRectF = new Paint();       //画矩形
        paintRectF.setColor(colors.get(2));
    ...
    private void drawAxesLine(Canvas canvas, Paint paint) {
        ...
        // 画 Y 轴
        canvas.drawLine(xPoint + 20, yPoint - 20, xPoint + 20, margin / 7 - 20, paint);
    }
